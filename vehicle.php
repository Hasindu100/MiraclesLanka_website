...
<body>
  ...
  <div class="row">
    ...
    <div class="container-6">
            ...
            <?php
                include 'config.php';
                $query = $db->query("SELECT * FROM vehicle ORDER BY vehicle.uploaded_on DESC");
                if($query->num_rows > 0){
                    while($row = $query->fetch_assoc()){
                        $imageURL = 'uploads/'.$row["file_name"];
                        $output = '<div class="col-md-12">
                        <div class="card mb-2">
                        <b style="font-size:20px;color:gray">'.$row["title"].'</b>
                            <img class="card-img-top"
                                src="'.$imageURL.'" alt="Card image cap">
                            <div class="card-body">
                                <h4 class="card-title" style="color:#1d4704">RS.'.$row["price"].'</h4> 
                                <h5 class="card-title" style="color:#bbb">'.$row["location"].'</h5>
                                
                                <a href="vehicleinfo.php?vehicle_id='.$row["vehicle_id"].'" class="btn btn-success">View more>></a>
                            </div>
                        </div>
                    </div>';
                    echo $output;
